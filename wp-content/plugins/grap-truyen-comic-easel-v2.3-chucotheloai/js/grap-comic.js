jQuery(document).ready(function () {
    ...
    // Check grap click function
    jQuery('.check-grap').click(function () {

        jQuery(this).val('Đang kiểm tra...');
        ...
        var ten_truyen = jQuery('#title').val();
        ...
        jQuery.post(GRAP_AJ.ajaxurl, data, function (response) {
            jQuery('.check-grap').val('Kiểm tra');
    ...
    jQuery(document).on("click", ".click-to", function () {
        ...
        var tenchuong = jQuery(this).data('tenchuong');
        var quyen = jQuery(this).data('quyen');
        var img_link = jQuery(this).data('img');
            ...
            if (checked == true) {
                ...
                jQuery.post(GRAP_AJ.ajaxurl, data, function (response) {
                    ...
                    if (response == '0') {
                        ...
                        grap_anh_truyen(nguon_truyen, post_title);
                    }
                });
            }
...
jQuery('.grap-multi-btn').click(function () {
    ...
    jQuery(jQuery(".grap-result input[type='checkbox']:checked:not(:disabled)").get().reverse()).each(function () {
        ...
        tenchuong.push(jQuery(this).data('tenchuong'));
    ...
    var i = 0;
    insert_multi_chap(link_chap[0], html_noi_dung_truyen[0], nguon_truyen[0], post_title, chap_title[0], parent[0], tenchuong[0]);

    function insert_multi_chap(link_chap, html_noi_dung_truyen, nguon_truyen, post_title, chap_title, parent, tenchuong) {
    ...
    function call_back_ajax() {
        if (i < count) {
            insert_multi_chap(link_chap[i], html_noi_dung_truyen[i], nguon_truyen[i], post_title, chap_title[i], parent[i], tenchuong[i]);
        }
        else {
            jQuery('.grap-multi-btn').val('Lấy những chap đã chọn');
            jQuery('.grap-spinner').hide().fadeOut('slow');
        }
    }
...
function grap_anh_truyen(nguon_truyen, ten_truyen)
{
    //var nguon_truyen 			= jQuery('.nguon_truyen select').val();
    var link_truyen = jQuery('.link_truyen input').val();
    ...
    if (nguon_truyen == "")
    {
        alert("Vui lòng chọn nguồn truyện");
        return false;
    }
    if (link_truyen == "")
    {
    ...
    layanhdaidien(link_truyen, nguon_truyen, ten_truyen);
}
function layanhdaidien(link_truyen, nguon_truyen, ten_truyen)
{
    jQuery.ajax({
        url: GRAP_AJ.ajaxurl,
        data: {
            'action': 'grap_anhdaidien',
            'link_truyen': link_truyen,
            'nguon_truyen': nguon_truyen,
            'ten_truyen': ten_truyen
