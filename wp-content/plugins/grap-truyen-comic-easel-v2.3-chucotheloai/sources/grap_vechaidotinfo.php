...
function grap_vechaidotinfo($link_truyen, $truyen_co_nhieu_chap, $loai_truyen, $ten_truyen) {
	?>
	<div class="postbox"><?php

		
			
			get_list_vechaidotinfo($link_truyen, $ten_truyen);
...
function get_list_vechaidotinfo($link_truyen, $ten_truyen) {

		$array_ten_chap = check_chap_exists($ten_truyen);
		
		//echo get_single_content_vechaidotinfo('http://vechai.info/hen-koi-after-school-diary/chap-003');

		$html = file_get_html($link_truyen);
				
		//$content = $html->find("#mainStory",0)->innertext;
		$content =$html->find("#chapterList .inner",1);
		//echo $content->find('li',0);
		$list_link=array();
		
		foreach($content->find('li') as $chaps)
		{	$href=$chaps->find('ul li a',0)->href;
			
			if($href!='')
			{
			$array_chap=array();
			$array_chap['href']=$href;
			$array_chap['title']=$chaps->find('ul li a',0)->innertext;
...
function get_single_content_vechaidotinfo($link_chap) {

	$content 	= "";
	$html 	= file_get_html($link_chap);
	$content = $html->find("#contentChapter", 0);
