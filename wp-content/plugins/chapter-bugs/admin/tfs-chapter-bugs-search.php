...
function chapter_bugs_search() {
	global $tfscb, $tfscb_db;
    ...
    $like = $_POST['s'];
	$chapter_bugs_list = $tfscb_db->find_all_chapter_bugs_search( $like, $orderby, $order, $items_per_page, $start );
	...
	<div class="wrap">
		...
		<form id="list-chapter-bugs" class="tfs-chapter-bugs-form" method="POST" action="<?php echo $tfscb->tfscb_chapter_bugs->base_page . '&amp;paged=' . $_GET['paged']; ?>" accept-charset="utf-8">
			...
			<div class="tablenav top">
	        	...
	        	<?php $tfscb->tfscb_chapter_bugs->pagination( 'top', $_GET['paged'], $tfscb->paged['total_objects'], $tfscb->paged['objects_per_page']  ); ?>
			</div>
			<table class="wp-list-table widefat fixed" cellspacing="0">
				...
				<?php
					if($chapter_bugs_list) {
						...
						foreach($chapter_bugs_list as $chapter_bugs) {
			...
			<?php $tfscb->tfscb_chapter_bugs->pagination( 'bottom', $_GET['paged'], $tfscb->paged['total_objects'], $tfscb->paged['objects_per_page']  ); ?>
	        </div>
		</form>
	</div>
<?php
...
class Tfs_Chapter_Bugs_List_Table extends WP_List_Table {
	...
	function get_columns() {
    	$columns = array();
    	$columns['cb'] 			= '<input name="checkall" type="checkbox" onclick="check_all(document.getElementById(\'edit-untag-me\'));" />';
    	$columns['user_id'] 	= __('User Id');
    	$columns['error_link'] 	= __('Error Link');
    	$columns['comment'] 	= __('Comment');
    	$columns['status'] 		= __('Status');
    	$columns = apply_filters('vcx_user_membership_chapter_bugs_columns', $columns);
    	return $columns;
	}
